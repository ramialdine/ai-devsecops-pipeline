...
pip-audit, ZAP), calls the Claude API to classify and deduplicate findings,
...
def triage_findings(findings: list[dict]) -> list[dict]:
    """Send findings to Claude for deduplication, classification, and remediation."""
    if not findings:
        return []
...
def format_pr_comment(triaged: list[dict]) -> str:
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Generated by [ai-devsecops-pipeline](https://github.com) — Claude-powered triage*")
...
def main():
    ...
    if not findings:
        ...
        print("[triage] Sending findings to Claude for triage...", file=sys.stderr)
